...
def build_cifar_resnet18():
    ...
    model.maxpool = (
        nn.Identity()
    )  # Remove initial maxpool to preserve spatial resolution
...
for epoch in range(1, EPOCHS + 1):
    ...
    if epoch % 10 == 0 or epoch == EPOCHS:
        ...
        print(
            f"[{elapsed}] Epoch {epoch:03d}/{EPOCHS:03d} | Train Loss: {train_loss:.4f} | Train Acc: {train_acc:.2f}% | Val Acc: {val_acc:.2f}%{saved_str}"
        )
