...
def create_sub_resnet(teacher, kept1, kept2, kept3, kept4):
    ...
    def slice_basic_block(src, dst, in_m, out_m, is_first_block):
        ...
        if len(src.shortcut) > 0:
            dst.shortcut[0].weight.data.copy_(
                src.shortcut[0].weight.data[out_m][:, prev_m]
            )
            ...
            dst.shortcut[1].running_mean.data.copy_(
                src.shortcut[1].running_mean.data[out_m]
            )
            dst.shortcut[1].running_var.data.copy_(
                src.shortcut[1].running_var.data[out_m]
            )

    in_m_0 = np.arange(64)
    slice_basic_block(
        teacher.layer1[0], sub_net.layer1[0], in_m_0, m1, is_first_block=True
    )
    slice_basic_block(
        teacher.layer1[1], sub_net.layer1[1], m1, m1, is_first_block=False
    )

    slice_basic_block(teacher.layer2[0], sub_net.layer2[0], m1, m2, is_first_block=True)
    slice_basic_block(
        teacher.layer2[1], sub_net.layer2[1], m2, m2, is_first_block=False
    )

    slice_basic_block(teacher.layer3[0], sub_net.layer3[0], m2, m3, is_first_block=True)
    slice_basic_block(
        teacher.layer3[1], sub_net.layer3[1], m3, m3, is_first_block=False
    )

    slice_basic_block(teacher.layer4[0], sub_net.layer4[0], m3, m4, is_first_block=True)
    slice_basic_block(
        teacher.layer4[1], sub_net.layer4[1], m4, m4, is_first_block=False
    )
...
def run_cifar_seed_benchmark(seed_idx, seed):
    ...
    print(
        f"{get_elapsed_str()}   [Step 1/3] Training ResNet-18 Teacher Baseline (15 Epochs)..."
    )
    teacher_model.train()
    ...
    # -----------------------------------------------------------------
    print(
        f"{get_elapsed_str()}   [Step 2/3] Profiling Causal Stage Channels & Computing Fingerprints..."
    )
    ...
    with torch.no_grad():
        for i in range(64):
            temp_s1 = s1_act.clone()
            temp_s1[:, i, :, :] = 0.0
            out = teacher_model.fc(
                torch.flatten(
                    teacher_model.avgpool(
                        teacher_model.layer4(
                            teacher_model.layer3(teacher_model.layer2(temp_s1))
                        )
                    ),
                    1,
                )
        ...
        for j in range(128):
            temp_s2 = s2_act.clone()
            temp_s2[:, j, :, :] = 0.0
            out = teacher_model.fc(
                torch.flatten(
                    teacher_model.avgpool(
                        teacher_model.layer4(teacher_model.layer3(temp_s2))
                    ),
                    1,
                )
            )
        ...
        for k in range(256):
            temp_s3 = s3_act.clone()
            temp_s3[:, k, :, :] = 0.0
            out = teacher_model.fc(
                torch.flatten(teacher_model.avgpool(teacher_model.layer4(temp_s3)), 1)
            )
            scores_s3[k] = (
                (1.0 - F.cosine_similarity(val_teacher_logits, out, dim=1))
                .mean()
    ...
    def build_conv_core(target_channels, use_causal=True):
        ...
        if len(rem1) > 0 and k_c1 > 0:
            ...
            for cid in range(k_c1):
                ...
                kept1.add(rep)
                t_model.layer1[1].conv2.weight.data[rep] = (
                    t_model.layer1[1].conv2.weight.data[m].mean(dim=0)
                )
                t_model.layer1[1].bn2.weight.data[rep] = (
                    t_model.layer1[1].bn2.weight.data[m].mean(dim=0)
                )
                t_model.layer1[1].bn2.bias.data[rep] = (
                    t_model.layer1[1].bn2.bias.data[m].mean(dim=0)
                )
                norm = np.sqrt(len(m))
                for x in m:
                    if x != rep:
                        t_model.layer2[0].conv1.weight.data[:, rep] += (
                            t_model.layer2[0].conv1.weight.data[:, x] / norm
                        )
                        if len(t_model.layer2[0].shortcut) > 0:
                            t_model.layer2[0].shortcut[0].weight.data[:, rep] += (
                                t_model.layer2[0].shortcut[0].weight.data[:, x] / norm
                            )
        ...
        if len(rem2) > 0 and k_c2 > 0:
            ...
            for cid in range(k_c2):
                ...
                kept2.add(rep)
                t_model.layer2[1].conv2.weight.data[rep] = (
                    t_model.layer2[1].conv2.weight.data[m].mean(dim=0)
                )
                t_model.layer2[1].bn2.weight.data[rep] = (
                    t_model.layer2[1].bn2.weight.data[m].mean(dim=0)
                )
                t_model.layer2[1].bn2.bias.data[rep] = (
                    t_model.layer2[1].bn2.bias.data[m].mean(dim=0)
                )
                norm = np.sqrt(len(m))
                for x in m:
                    if x != rep:
                        t_model.layer3[0].conv1.weight.data[:, rep] += (
                            t_model.layer3[0].conv1.weight.data[:, x] / norm
                        )
                        if len(t_model.layer3[0].shortcut) > 0:
                            t_model.layer3[0].shortcut[0].weight.data[:, rep] += (
                                t_model.layer3[0].shortcut[0].weight.data[:, x] / norm
                            )
        ...
        if len(rem3) > 0 and k_c3 > 0:
            ...
            for cid in range(k_c3):
                ...
                kept3.add(rep)
                t_model.layer3[1].conv2.weight.data[rep] = (
                    t_model.layer3[1].conv2.weight.data[m].mean(dim=0)
                )
                t_model.layer3[1].bn2.weight.data[rep] = (
                    t_model.layer3[1].bn2.weight.data[m].mean(dim=0)
                )
                t_model.layer3[1].bn2.bias.data[rep] = (
                    t_model.layer3[1].bn2.bias.data[m].mean(dim=0)
                )
                norm = np.sqrt(len(m))
                for x in m:
                    if x != rep:
                        t_model.layer4[0].conv1.weight.data[:, rep] += (
                            t_model.layer4[0].conv1.weight.data[:, x] / norm
                        )
                        if len(t_model.layer4[0].shortcut) > 0:
                            t_model.layer4[0].shortcut[0].weight.data[:, rep] += (
                                t_model.layer4[0].shortcut[0].weight.data[:, x] / norm
                            )

        # Stage 4 Clustering
        if len(rem4) > 0 and k_c4 > 0:
            c4 = AgglomerativeClustering(
                n_clusters=min(k_c4, len(rem4)), metric="cosine", linkage="average"
            )
            ...
            for cid in range(k_c4):
                ...
                kept4.add(rep)
                t_model.layer4[1].conv2.weight.data[rep] = (
                    t_model.layer4[1].conv2.weight.data[m].mean(dim=0)
                )
                t_model.layer4[1].bn2.weight.data[rep] = (
                    t_model.layer4[1].bn2.weight.data[m].mean(dim=0)
                )
                t_model.layer4[1].bn2.bias.data[rep] = (
                    t_model.layer4[1].bn2.bias.data[m].mean(dim=0)
                )
                norm = np.sqrt(len(m))
                for x in m:
                    if x != rep:
                        t_model.fc.weight.data[:, rep] += (
                            t_model.fc.weight.data[:, x] / norm
                        )
    ...
    for b in target_budgets:
        ...
        print(
            f"{get_elapsed_str()}     -> Budget {pct:4.1f}% ({total_k:3d} ch) | Our FT: {o_fa:.2f}% | DA FT: {d_fa:.2f}% ({b_time:.1f}s)"
        )
...
print(
    f"{get_elapsed_str()} Benchmark complete. Output plot saved to 'cifar10_resnet18_benchmark.png'"
)
