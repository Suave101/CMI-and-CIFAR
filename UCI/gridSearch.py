...
print(f"{'Retention %':<12} | {'Kept Neurons':<12} | {'Test Acc %':<12} | {'Agreement %':<12} | {'Batch MFLOPs':<12}")
print("-" * 72)
...
for pct in retention_percentages:
    ...
    print(f"{pct:<12}% | {total_retained:<12} | {acc:<12.2f}% | {agree:<12.2f}% | {total_mflops:<12.3f} MFLOPs")
...
line1 = ax1.plot(retention_percentages, accuracies, 'o-', color=color_acc, linewidth=2, label='Test Accuracy (%)')
line2 = ax1.plot(retention_percentages, agreements, 's--', color=color_agree, linewidth=2, label='Prediction Agreement (%)')
...
ax2 = ax1.twinx()
color_flops = 'tab:red'
line3 = ax2.plot(retention_percentages, batch_mflops, 'd-.', color=color_flops, linewidth=2, label='Batch FLOPs (MFLOPs)')
...
plt.title('Trade-off Study: Retention % vs Accuracy, Agreement & Computational FLOPs', fontsize=14)
plt.tight_layout()
